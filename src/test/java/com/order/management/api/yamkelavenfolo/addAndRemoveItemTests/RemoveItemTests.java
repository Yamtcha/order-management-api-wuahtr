...
public class RemoveItemTests {
    ...
    public void itemOrderNotFoundTest() {
        ...
        RuntimeException exception = assertThrows(RuntimeException.class, () -> {
            orderService.decreaseItemQuantity("1234567845","play station");
        });

        assertEquals(exception.getMessage(),"Order=" + "1234567845" + " doesn't exist");
    }
    ...
    public void itemNotFoundTest() {
        Mockito.when(ordersDAO.selectOrders()).thenReturn(ordersList);
        RuntimeException exception = assertThrows(RuntimeException.class, () -> {
            orderService.decreaseItemQuantity("545631539154165","laptop");
        });
